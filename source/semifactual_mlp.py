...
# Inner Module for MLP branch
class EdgeMaskPredictor(torch.nn.Module):
    def __init__(self, graph, mlp_hidden_dim=32):
        super().__init__()
        self.graph = graph
        
        # MLP to predict mask value from concatenated node embeddings
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(graph.num_features * 2, mlp_hidden_dim),
            torch.nn.ReLU(),
    ...
    def reset_parameters(self):
        """Reset parameters MLP components."""
    ...
    def forward(self, target_edge_indices):        
        ...
        feat_src = self.graph.x[src_nodes].float()
        feat_dst = self.graph.x[dst_nodes].float()

        # Concatenate node features for edges
        concatenated_features = torch.cat([feat_src, feat_dst], dim=-1)

        # Pass through MLP to get raw mask logits
        mask_logits = self.mlp(concatenated_features)
...
class ExplainModelGraph(torch.nn.Module):
    ...
    def __init__(self, graph, pred_orig, device, base_model, gnn_type, num_classes):
        ...
        self.remove_predictor = EdgeMaskPredictor(graph=self.graph).to(device)
        self.add_predictor = EdgeMaskPredictor(graph=self.graph).to(device)
...
# Logging.
result_folder = f'data/{args.dataset}/semifactual_mlp/'
