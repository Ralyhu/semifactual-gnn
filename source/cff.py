...
class GraphExplainerEdge(torch.nn.Module):
    ...
    def explain_dataset(self):
        ...
        return exps, cfs, total_sufficiency / len(cfs), total_necessity / len(cfs), total_size / len(cfs), explanation_times
...
dataset = dataset[indices[2]] # to compare with SEMIFACTUAL
...
if args.robustness == 'na':
    ...
    exps, cfs, sufficiency, necessity, average_size, explanation_times = explainer.explain_dataset()
    
    # Save explanation times to file as text
    times_save_path = os.path.join(result_folder, f'explanation_times_{args.gnn_type}_run_{args.explainer_run}.txt')
    avg_time = sum(explanation_times) / len(explanation_times)
    std_time = np.std(explanation_times)
    with open(times_save_path, 'w') as f:
