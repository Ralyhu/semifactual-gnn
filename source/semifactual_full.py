...
from gnn_trainer import GNNTrainer, GNN
import json  # Add this import


# Inner Module for GNN + MLP branch
class EdgeMaskPredictor(torch.nn.Module):
    def __init__(self, graph, gnn_type, num_classes, gnn_hidden_dim=20, mlp_hidden_dim=32):
        super().__init__()
        self.graph = graph
        # GNN to get node embeddings
        self.gnn = GNN(num_features=graph.num_features, 
                       num_classes=num_classes,
                       layer=gnn_type
                       ) # other parameters are set to default values as training the GNN to explain

        # MLP to predict mask value from concatenated node embeddings
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(gnn_hidden_dim * 2, mlp_hidden_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(mlp_hidden_dim, 1)
            # Sigmoid will be applied later
        )

        self.reset_parameters()

    def reset_parameters(self):
        """Reset parameters of both the GNN and MLP components."""
        # Reset GNN parameters using its own method
        self.gnn.reset_parameters()
        
        # Reset MLP parameters using standard initialization
        for module in self.mlp:
            if hasattr(module, 'reset_parameters'):
                module.reset_parameters()

    def forward(self, target_edge_indices):
        # Get node embeddings from GNN (we only need the first output)
        node_embeddings, _, _ = self.gnn(self.graph) # Pass data object, edge_weight is None as the graph has only the needed edges

        # Select embeddings for the target edges
        src_nodes = target_edge_indices[0]
        dst_nodes = target_edge_indices[1]

        # Concatenate node embeddings for edges
        emb_src = node_embeddings[src_nodes]
        emb_dst = node_embeddings[dst_nodes]
        concatenated_embeddings = torch.cat([emb_src, emb_dst], dim=-1)

        # Pass through MLP to get raw mask logits
        mask_logits = self.mlp(concatenated_embeddings)
        return mask_logits.squeeze(-1) # Return raw logits

# This class is responsible for explaining a single graph.
class ExplainModelGraph(torch.nn.Module):

    def __init__(self, graph, pred_orig, device, base_model, gnn_type, num_classes):
        ...
        # here I pass the graph object before creating the graph with added edges
        self.remove_predictor = EdgeMaskPredictor(graph=self.graph, gnn_type=self.gnn_type, num_classes=self.num_classes).to(device)
        self.add_predictor = EdgeMaskPredictor(graph=self.graph, gnn_type=self.gnn_type, num_classes=self.num_classes).to(device)
        ...
        self.num_unique_editable_edges = masked_edges_count
        self.current_final_unique_logits = None # Will be set by get_masked_adj

        if self.num_unique_editable_edges == 0:
    ...
    def get_masked_adj(self):
        # Run GNNs and MLPs for both branches
        raw_add_mask_logits = self.add_predictor(self.edges_to_add)
        raw_remove_mask_logits = self.remove_predictor(self.edges_to_remove)

        # Initialize a list to store logits for unique editable edges in the correct order
        logit_list = [torch.tensor(0.0, device=self.device) for _ in range(self.num_unique_editable_edges)]

        # --- Process remove_predictor logits ---
        temp_remove_logits_collector = {} 
        if self.edges_to_remove.numel() > 0:
            for i in range(self.edges_to_remove.size(1)):
                u, v = self.edges_to_remove[0, i].item(), self.edges_to_remove[1, i].item()
                edge_key = (min(u, v), max(u, v))
                temp_remove_logits_collector.setdefault(edge_key, []).append(raw_remove_mask_logits[i])
        
        avg_remove_logits = {}
        for key, logits_list_val in temp_remove_logits_collector.items():
            if logits_list_val:
                avg_remove_logits[key] = torch.stack(logits_list_val).mean()

        # --- Process add_predictor logits ---
        temp_add_logits_collector = {}
        if self.edges_to_add.numel() > 0:
            for i in range(self.edges_to_add.size(1)):
                u, v = self.edges_to_add[0, i].item(), self.edges_to_add[1, i].item()
                edge_key = (min(u, v), max(u, v))
                temp_add_logits_collector.setdefault(edge_key, []).append(raw_add_mask_logits[i])

        avg_add_logits = {}
        for key, logits_list_val in temp_add_logits_collector.items():
            if logits_list_val:
                avg_add_logits[key] = torch.stack(logits_list_val).mean()

        # --- Populate logit_list for final_unique_logits ---
        # Add-context logits take precedence
        for edge_key, param_idx in self.edge_to_param_idx.items():
            if edge_key in avg_add_logits:
                logit_list[param_idx] = avg_add_logits[edge_key]
            elif edge_key in avg_remove_logits:
                logit_list[param_idx] = avg_remove_logits[edge_key]
            # else: logit remains 0.0 (default from initialization)

        self.current_final_unique_logits = torch.stack(logit_list)
        
        # Create a mask tensor of zeros with length equal to the number of edges in the graph
        num_edges = self.graph.edge_index.size(1)
        edge_mask = torch.zeros(num_edges, device=self.device)
        
        # If no unique editable edges, return all zeros mask (though __init__ should raise error)
        if self.num_unique_editable_edges == 0:
            return edge_mask # Or handle as an error state if __init__ check is bypassed
        
        # Apply sigmoid to the collected logits for unique editable edges
        mask_values_for_unique_edges = torch.sigmoid(self.current_final_unique_logits)
        
        # Process edges that need masking (map unique edge masks to full graph edge_mask)
        for edge_idx in range(num_edges):
            ...
            if edge_key in self.edge_to_param_idx: 
                param_idx = self.edge_to_param_idx[edge_key]
                edge_mask[edge_idx] = mask_values_for_unique_edges[param_idx]
    ...
    def debug_check_mask_values(self, masked_adj, expected_values=None):
        ...
        """
        if not hasattr(self, 'current_final_unique_logits') or self.current_final_unique_logits is None or self.current_final_unique_logits.numel() == 0:
            return
            
        if expected_values is None:
            expected_values = torch.sigmoid(self.current_final_unique_logits).detach().cpu().numpy()
    ...
    def loss(self, prob_original_class, prob_different_class, gam, lambda_param):
        ...
        # Since current_final_unique_logits already contains only parameters for editable edges
        # We can directly compute the mean from these values after sigmoid
        if self.current_final_unique_logits is not None and self.current_final_unique_logits.numel() > 0:
            L1 = 1.0 - torch.mean(torch.sigmoid(self.current_final_unique_logits))
        else:
            L1 = torch.tensor(0.0, device=self.device) # Should not happen if num_unique_editable_edges > 0
    ...
    def forward(self, base_model, masked_adj=None):
        ...
        if masked_adj is None:
            # This call will set self.current_final_unique_logits
            masked_adj_full = self.get_masked_adj()
        else:
            masked_adj_full = masked_adj
            # If masked_adj is provided, self.current_final_unique_logits might not be from the latest predictors' state
            # However, loss calculation only happens when masked_adj is None (during training)
        
        # Run debug check if needed
        if training:
            self.debug_check_mask_values(masked_adj_full)  # Pass masked_adj_full here

        edge_weights = self.build_edge_weights(masked_adj_full) # Use masked_adj_full here
        ...
        return prob_original_class, prob_different_class, masked_adj_full # Return masked_adj_full
    ...
    def get_parameter_info(self):
        ...
        if not hasattr(self, 'current_final_unique_logits') or self.current_final_unique_logits is None:
            return param_info_list # Return empty if logits not computed yet

        # Ensure logits are on CPU for easy access
        edge_logits = self.current_final_unique_logits.cpu().detach()
        mask_values = torch.sigmoid(edge_logits)
        ...
        for edge_key, param_idx in self.edge_to_param_idx.items():
            raw_value = edge_logits[param_idx].item() # This is the logit value
...
class GraphExplainerEdge(torch.nn.Module):
    ...
    def __init__(self, base_model, G_dataset, args, device, gnn_type, num_classes, use_last):
    ...
    def explain_dataset(self, perturb_perc: int = 25):
        ...
        all_l1_losses = []
        all_total_losses = []
        # Initialize lists to store edit information
        all_edges_added = []
        all_edges_removed = []
        ...
        for g in tqdm(self.G_dataset, desc='Graph'):
            ...
            masked_adj, binarized_mask, irrelevance, size, explainer, loss_metrics = self.explain(graph_modified, pred_orig, self.gnn_type)
            ...
            # Append loss metrics
            all_semi_losses.append(loss_metrics['semi_losses'])
            all_l1_losses.append(loss_metrics['l1_losses'])
            all_total_losses.append(loss_metrics['total_losses'])
            
            # Append edit information
            edit_info = loss_metrics['edit_info']
            ...
            all_best_epochs.append(loss_metrics['best_epoch'])
    ...
    def explain(self, g, pred_orig, gnn_type):
...
# Logging.
result_folder = f'data/{args.dataset}/semifactual_full/'
