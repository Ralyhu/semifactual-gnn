...
parser.add_argument('--epochs', type=int, default=100, help='Number of epochs. Default is 1.')
...
if args.robustness == 'na':
    ...
    # Add downstream training time if stage 2
    if args.stage == 2:
        start_time = time.time()
        mlp = train_MLP(model, trainer.dim, device, train_loader, valid_loader, save_to=args.best_downstream_mlp_model_path)
        downstream_training_time = time.time() - start_time
        training_time += downstream_training_time
        mlp_explainer = MLPExplainer(mlp, device=device)
